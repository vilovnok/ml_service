...
from models.account import AccountUserID
    ...
    async def send_verify_code(self, uow: IUnitOfWork, data: VerifyCreateV2):
        try:
            async with uow:
                ...
                model = AccountUserID(user_id=data.user_id)                    

                if int(verify.code) != int(data.code):
                    await uow.rollback()
                ...
                await uow.user.update(where=[Users.id == verify.user_id], values={'is_verified': True}, n_tab=0)
                await uow.verify.update(where=[Verify.user_id == verify.user_id, Verify.is_active == True], values={'is_active': True}, n_tab=0)                
                await uow.account.add_one(model.model_dump(), n_tab=0)
                await uow.commit()                
